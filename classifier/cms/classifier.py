...
class Classifying():
        ...
        for cat in self.categories:
            ...
            for word in words:
                if word in word_count[cat]:
                    word_probs[cat][word] = word_count[cat][word] / cat_count[cat]
                else:
                    #ラプラススムージング
                    word_probs[cat][word] = 1 / cat_count[cat]  #pickleできる形式の制限からlambdaを使うdefaultdictは使えず
                    #全部else側にきてない？
    ...
    def save_article(self,title,url,category):#url,タイトル、カテゴリをデータベースに保存する。
        from cms.models import Article #classifier.pyからimportするevaluationをコマンドプロンプトから起動する関係で、この位置にしてある
        Article.objects.create(title=title, url=url, category=category)#データベースに保存
